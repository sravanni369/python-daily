...
| 15 | day15_gil_threads_vs_processes.py | The global interpreter lock measured (Dive into Deep Learning, p. 270): the same CPU-bound work run serially, on threads and on processes, where threads come out at 0.90x, slower than doing nothing, while processes give 2.08x, then the same three ways on I/O-bound work where the same threads give 3.97x. Same tool, opposite result, depending on what the work is waiting for |
