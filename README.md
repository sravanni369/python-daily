...
| 15 | day15_gil_threads_vs_processes.py | The global interpreter lock measured (Dive into Deep Learning, p. 270): the same CPU-bound work run serially, on threads and on processes, where threads give 1.00x and processes 2.40x, then the same three ways on I/O-bound work where threads give 3.92x. Same tool, opposite result, depending on what the work is waiting for |
